...
WITH_CONDITIONAL_PROBABILITY = False
WITH_LC = False
WITH_AVERAGE = False
...
class Influencers:
    ...
    def get_num_with_multiplication(self, without=None):
        if WITH_AVERAGE:
            return self.get_num_distinct(without=without)
        if without is None:
    ...
    def common_num_with_multiplication(self, other, without=None):
        ...
        for infl in self.infl_dict.keys():
            if infl in other.infl_dict:
                if without is None or infl != without:
...
class Variable(TreeNode):
    ...
    def calculate_gamma_delta(self, feature, features_sample, special_probs):
        ...
        if not WITH_CONDITIONAL_PROBABILITY:
            if self.visited:
...
class Constant(Leaf):
    def __init__(self, id, const_value):
        Leaf.__init__(self, id)
        assert const_value==0 or const_value==1
        self.const_value = const_value

    def print(self, level):
        print(' ' * (level * 4) + "Const " + str(self.id) + ": " + str(self.const_value))

    def calculate_influencers(self):
        pass

    def calculate_gamma_delta(self, feature, features_sample, special_probs):
        assert self.influencers.get_num_with_multiplication() == 0
        self.gamma = [self.const_value]
        self.delta = [self.const_value]
...
class AndGate(TreeNode):
    ...
    def calculate_gamma_delta(self, feature, features_sample, special_probs):
        # assert (self.children[0].influencers.common_num_with_multiplication(self.children[1].influencers) == 0 or self.children[0].influencers.common_num_with_multiplication(self.children[1].influencers) == self.children[0].influencers.get_num_with_multiplication() + self.children[1].influencers.get_num_with_multiplication()) # assumption FIXME

        # assert special_probs == {} or len(self.children[0].influencers & self.children[1].influencers) == 0  # assumption FIXME

        if WITH_CONDITIONAL_PROBABILITY and self.children[0].influencers.common_num_with_multiplication(self.children[1].influencers) > 0: # All are common FIXME
            ...
            for influencer in self.children[0].influencers.get_distinct_list(without=feature):
                if not self.children[1].influencers.is_in(influencer):
                    continue
        ...
        if WITH_AVERAGE and self.children[0].influencers.common_num_with_multiplication(self.children[1].influencers) > 0:
            eldar = 1
            assert self.children[0].influencers.get_distinct_list() == self.children[1].influencers.get_distinct_list()
            #assert len(self.children[0].gamma) == len(self.children[1].gamma)

            self.gamma = [0]*(self.children[0].influencers.get_num_with_multiplication(without=feature)+1)
            self.delta = [0]*(self.children[0].influencers.get_num_with_multiplication(without=feature)+1)

            for k in range(self.children[0].influencers.get_num_with_multiplication(without=feature)+1):
                if self.children[0].gamma[k] is None:
                    assert self.children[1].gamma[k] is None
                    assert self.children[0].delta[k] is None
                    assert self.children[1].delta[k] is None
                    continue
                self.gamma[k] = 0.5*(self.children[0].gamma[k]+self.children[1].gamma[k])
                self.delta[k] = 0.5*(self.children[0].delta[k]+self.children[1].delta[k])
        ...
        if WITH_CONDITIONAL_PROBABILITY and self.children[0].influencers.common_num_with_multiplication(self.children[1].influencers) > 0:
            for influencer in special_probs_given_child0_1:
                if self_gamma_always_zero:
                    for l in range(l_range + 1):
                        self.gamma[l] = 0
                if self_delta_always_zero:
                    for l in range(l_range + 1):
                        self.delta[l] = 0
    ...
    def calculate_prob_res_given_infl_1_and_feature_value(self, feature, feature_value, infl, res):
        ...
        if self.children[0].influencers.is_in(infl):
            infl_child = self.children[0]
            other_child = self.children[1]
        elif self.children[1].influencers.is_in(infl):
    ...
    def simulate(self, leaves_sample) -> bool:
        child_0_sim = self.children[0].simulate(leaves_sample)
        if not child_0_sim:
            return False

        return self.children[1].simulate(leaves_sample)
...
class NotGate(TreeNode):
    ...
    def calculate_gamma_delta(self, influencer, features_sample, special_probs):
        ...
        for l in range(self.influencers.get_num_with_multiplication(without=influencer)+1):
            comb = math.comb(self.influencers.get_num_with_multiplication(without=influencer), l)
    ...
    def simulate(self, leaves_sample) -> bool:
        return not self.child.simulate(leaves_sample)
...
class BinaryCircuit:
    ...
    def _add_output_or_bad_state_or_latch_next(self, literal: int, is_output: bool, is_bad_state: bool = False):
        assert literal // 2 in self.vars or literal==0 or literal==1
        ...
        if literal == 0 or literal == 1:
            my_output.child = Constant('c0', literal)
        elif literal % 2 == 0:
    ...
    def calculate_shap_scores(self, features, features_sample, output_to_check, prints=True):
        ...
        for feature in features:
            ...
            # print("Top gamma: ", output_node.gamma)
            # print("Top delta: ", output_node.delta)
            ...
            for k in range(coi_features_num):
                ...
                (features_sample[feature] - 0.5) * (output_node.gamma[k] - output_node.delta[k]))
            if prints:
                print("Current shap score: ", shap_scores[feature])
...
class AAGTree:
    ...
    def shap_scores(self, features, features_sample, output_to_check, prints=True):
        assert len(features_sample.keys())==self.inputs_num+self.latches_num
        return self.circuit.calculate_shap_scores(features, features_sample, output_to_check, prints)
