from Old.AAGTree import AAGTree
import gui
import numpy as np
...
def print_tree(aag_tree: AAGTree, output_to_print=None):
    if output_to_print:
        aag_tree.circuit.id_map[output_to_print].print()
    else:
        aag_tree.print()

def open_gui(aag_tree: AAGTree, init_sample={}):
...
def print_shap_chart(shap_scores, simulation, highlight_color='dodgerblue', color_positive='cornflowerblue', color_negative='violet'):
    ...
    print("sorted_shap_scores", sorted([item for item in shap_scores.items() if item[1]!=0], key=lambda x: x[1], reverse=True))
...
def calculate_shap_scores(tree, inputs_sample, outputs_to_check, prints=True):
    if prints:
